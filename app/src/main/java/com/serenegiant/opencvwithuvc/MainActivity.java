...
import com.serenegiant.uvccamera.R.xml;
...
import camera2basic.Camera2BasicFragment;
...
public final class 	MainActivity extends BaseActivity
     ...
     */
//    private static final int PREVIEW_WIDTH = 1920;//640
     ...
     * {@link UVCCamera#setPreviewSize(int, int, int)} throw exception
     */
//    private static final int PREVIEW_HEIGHT = 1080;//480
	...
	private Fragment builtInCameraFragment;
	...
	protected void onCreate(final Bundle savedInstanceState) {
		...
		if (null == savedInstanceState) {
			builtInCameraFragment = Camera2BasicFragment.newInstance();
			getFragmentManager().beginTransaction()
					.add(R.id.container, builtInCameraFragment)
					.show(builtInCameraFragment).commit();
...
//					List<DeviceFilter> filter = DeviceFilter.getDeviceFilters(MainActivity.this, xml.device_filter);
//					List<UsbDevice> deviceList = mUSBMonitor.getDeviceList((DeviceFilter)filter.get(0));
//					this.mDeviceListAdapter = new CameraDialog.DeviceListAdapter(MainActivity.this, mUSBMonitor.getDeviceList((DeviceFilter)filter.get(0)));
...
//						Toast.makeText(MainActivity.this, "deviceList.get(0):" + deviceList.get(0), Toast.LENGTH_LONG).show();
//						mUSBMonitor.requestPermission((UsbDevice)device);
//					}
//					CameraDialog.showDialog(MainActivity.this);
	...
	private void startPreview() {
		...
		fragmentTransaction.hide(builtInCameraFragment).commit();
	...
	private void stopPreview() {
		...
		try {
			FragmentManager fragmentManager = getFragmentManager();
			FragmentTransaction fragmentTransaction = fragmentManager.beginTransaction();
			fragmentTransaction.show(builtInCameraFragment).commitAllowingStateLoss();
			...
			Toast.makeText(MainActivity.this, stringWriter.toString(), Toast.LENGTH_LONG).show();
		}
	}
...
//			List<UsbDevice> deviceList = mUSBMonitor.getDeviceList((DeviceFilter)filter.get(0));
//			UsbDevice device = deviceList.get(0);
			Toast.makeText(MainActivity.this, "device" + device.toString(), Toast.LENGTH_LONG).show();
		...
		public void onDisconnect(final UsbDevice device,
			...
			Toast.makeText(MainActivity.this, "onDisconnect:", Toast.LENGTH_LONG).show();
			if (mCameraHandler != null) {
